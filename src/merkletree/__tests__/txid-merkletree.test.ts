...
describe('txid-merkletree', () => {
  ...
  it('Should get Txid merkletree DB paths', async () => {
    ...
    await Promise.all(
      vectors.map(async (vector) => {
        const merkletreeVectorTest = await TXIDMerkletree.createForWallet(
          db,
          vector.chain,
          txidVersion,
          async () => true,
  ...
  it('Should update railgun txid merkle tree correctly', async () => {
    const merkletree = merkletreeWallet;
    ...
    // Wallet merkletree does not store merkleroots
    ...
    const merkletree2 = await TXIDMerkletree.createForWallet(db, chain, txidVersion, async () => true);
    ...
    // Wallet merkletree uses current root for merkle proof
    ...
    // Wallet merkletree does not store historical merkleroots
