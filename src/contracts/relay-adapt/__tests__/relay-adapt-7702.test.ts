...
import { deriveEphemeralWallet } from '../../../key-derivation/ephemeral-key';
...
describe('relay-adapt-7702', function test() {
  this.timeout(45_000);
  ...
  beforeEach(async () => {
    ...
    await engine.loadNetwork(
      ...
      undefined,
      !isV2Test(), // supportsV3
      config.contracts.relayAdapt7702, // relayAdapt7702ContractAddress
    );
  ...
  it('[HH] Should wrap and shield base token', async function run() {
    ...
    // Manual 7702 Flow
    const contract7702 = RelayAdaptVersionedSmartContracts.getRelayAdaptContract(
      txidVersion,
      chain,
      true,
    ) as RelayAdapt7702Contract;

    // Ephemeral Key
    const ephemeralWallet = deriveEphemeralWallet(config.mnemonic, 0);

    // HACK: Set code of ephemeral address to RelayAdapt7702 code to simulate 7702 delegation
    const code = await provider.getCode(contract7702.address);
    await provider.send('hardhat_setCode', [ephemeralWallet.address, code]);

    const orderedCalls = await contract7702.getOrderedCallsForShieldBaseToken(
      shieldRequest,
    ...
    const signature = await RelayAdapt7702Helper.signExecutionAuthorization(
      ephemeralWallet,
      [], // transactions (empty for shield base token)
      actionData,
      BigInt(chain.id),
    );
    ...
    await Promise.all([
      shieldEventPromise,
      promiseTimeout(awaitScan(wallet, chain), 30000),
    ]);
    await wallet.refreshPOIsForTXIDVersion(chain, txidVersion, true);
  ...
  it('[HH] Should execute relay adapt transaction for unshield base token', async function run() {
    ...
    expect(relayAdaptParams).to.equal(
      '0x1aa79a9d04be5ee64c53f023bd2acb13409532c5b022ed90e1589d830cf8fdbb',
    );

    // 4. Create real transactions with relay adapt params.
    transactionBatch.setAdaptID({
      contract: ephemeralWallet.address,
    ...
    const callResultError = RelayAdapt7702Contract.getRelayAdaptCallError(txReceipt.logs);
  ...
  it('[HH] Should execute relay adapt transaction for NFT transaction', async function run() {
    ...
    const relayShieldInputs = await RelayAdapt7702Helper.generateRelayShieldRequests(
      shieldRandom,
      [],
    ...
    // Calculate params for signature
    const orderedCallsGasEstimate = await contract7702.getOrderedCallsForCrossContractCalls(
      crossContractCalls,
      relayShieldInputs
    );
    // HACK: Update to address for 7702
    orderedCallsGasEstimate.forEach((call) => {
      if (call.to === contract7702.address) {
        call.to = ephemeralWallet.address;
      }
    });
    ...
    const signatureGasEstimate = await RelayAdapt7702Helper.signExecutionAuthorization(
      ephemeralWallet,
      dummyTransactions,
      actionDataGasEstimate,
      BigInt(chain.id),
    );
    ...
    const populatedTransactionGasEstimate =
      await RelayAdaptVersionedSmartContracts.populateCrossContractCalls(
        ...
        true,
        authorization,
        signatureGasEstimate,
        ephemeralWallet.address
      );
    populatedTransactionGasEstimate.from = DEAD_ADDRESS;
    const gasEstimate = await RelayAdapt7702Contract.estimateGasWithErrorHandler(
      provider,
      populatedTransactionGasEstimate,
    );
    expect(Number(gasEstimate)).to.be.greaterThan(
      Number(
        RelayAdapt7702Contract.getMinimumGasLimitForContract(
    ...
    // Sign Execution Auth
    const orderedCallsFinal = await contract7702.getOrderedCallsForCrossContractCalls(
      crossContractCalls,
      relayShieldInputs
    );
    // HACK: Update to address for 7702
    orderedCallsFinal.forEach((call) => {
      if (call.to === contract7702.address) {
        call.to = ephemeralWallet.address;
      }
    });
    ...
    const signatureFinal = await RelayAdapt7702Helper.signExecutionAuthorization(
      ephemeralWallet,
      provedTransactions,
      actionDataFinal,
      BigInt(chain.id),
  ...
  it('[HH] Should shield all leftover WETH in relay adapt contract', async function run() {
    ...
    // Manual 7702 Shield of leftover
    const shield = new ShieldNoteERC20(
      wallet.masterPublicKey,
      SHIELD_RANDOM,
      0n, // Shield all
  ...
  it('[HH] Should execute relay adapt transaction for cross contract call', async function run() {
    ...
    const relayAdaptAddressBalance: bigint = await wethTokenContract.balanceOf(
      RelayAdaptVersionedSmartContracts.getRelayAdaptContract(txidVersion, chain, true).address,
    );
    expect(relayAdaptAddressBalance).to.equal(0n);
  ...
  it('[HH] Should revert send, but keep fees for failing cross contract call', async function run() {
    ...
    populatedTransactionGasEstimate.from = DEAD_ADDRESS;
    await expect(
      RelayAdapt7702Contract.estimateGasWithErrorHandler(provider, populatedTransactionGasEstimate),
  ...
  it('Should calculate relay adapt params', () => {
    ...
    const relayAdaptParams = RelayAdapt7702Helper.getRelayAdaptParams(
    ...
    const expectedParamsHex =
      ...
      bytesToHex(
        new Uint8Array([
          158, 121, 120, 133, 213, 222, 191, 122, 18, 206, 124, 208, 113, 253, 244, 194, 183, 180, 136,
          153, 29, 44, 89, 196, 146, 178, 37, 250, 159, 195, 7, 0,
        ]),
      );
  ...
  it('Should decode and parse relay adapt error logs (from failed Sushi V2 LP removal)', () => {
    const relayAdaptError = RelayAdapt7702Contract.getRelayAdaptCallError(
  ...
  it('Should extract call failed index and error message from ethers error', () => {
    ...
    const { callFailedIndexString, errorMessage } =
      RelayAdapt7702Contract.extractGasEstimateCallFailedIndexAndErrorText(errorText);
  ...
  it('Should parse relay adapt log revert data - relay adapt abi value', () => {
    const parsed = RelayAdapt7702Contract.parseRelayAdaptReturnValue(
